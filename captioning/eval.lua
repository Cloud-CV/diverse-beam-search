...
function TorchModel:__init(model, batch_size, num_images, language_eval, dump_images, dump_json, dump_json_postfix, dump_path, B, M, lambda, divmode, temperature, primetext, ngram_length, image_folder, image_root, input_h5, input_json, split, coco_json, backend, id, seed, gpuid, div_vis_dir)
...
function TorchModel:table_invert(t)
  -------------------------------------------------------------------------------
  -- invert vocab
  -------------------------------------------------------------------------------
  local s = {}
...
function TorchModel:convert_vocab_keys_to_int(vocab)
  local result = {}
...
function TorchModel:loadModel()
  ...
  if string.len(self.input_h5) == 0 then self.input_h5 = checkpoint.self.input_h5 end
  if string.len(self.input_json) == 0 then self.input_json = checkpoint.self.input_json end
  if self.batch_size == 0 then self.batch_size = checkpoint.self.batch_size end
  ...
  -------------------------------------------------------------------------------
  local loader
  if string.len(self.image_folder) == 0 then
    loader = DataLoader{h5_file = self.input_h5, json_file = self.input_json}
  else
    loader = DataLoaderRaw{folder_path = self.image_folder, coco_json = self.coco_json}
  end
...
-------------------------------------------------------------------------------
function TorchModel:eval_split(split, evalopt)
  local verbose = utils.getopt(evalopt, 'verbose', true)
  local num_images = utils.getopt(evalopt, 'num_images', true)

  self.protos.cnn:evaluate()
  self.protos.lm:evaluate()
  self.loader:resetIterator(split) -- rewind iteator back to first datapoint in the split
  local n = 0
  local loss_sum = 0
  local loss_evals = 0
  local final_beams = {}
  while true do
    ...
    local data = self.loader:getBatch{batch_size = self.batch_size, split = split, seq_per_img = self.checkpoint['seq_per_img']}
